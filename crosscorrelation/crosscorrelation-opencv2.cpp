...
int main(int argc, char **argv) {
  ...
  String comparisonMethods[] = {
      "CV_TM_SQDIFF", "CV_TM_SQDIFF_NORMED",
      "CV_TM_CCORR",  "CV_TM_CCORR_NORMED",
      "CV_TM_CCOEFF", "CV_TM_CCOEFF_NORMED"}; // List of comparison methods.
  ...
  Mat realMap = imread(
      realMapPath,
      -1); // Get the real map image. 0 is grayscale. -1 is original image.
  Mat slamMap = imread(
      slamMapPath,
      -1); // Get the slam map image. 0 is grayscale. -1 is original image.
  ...
  result.create(resultColumns, resultRows,
                CV_32FC1); // Allocate space for the result.
  ...
  if (templ.depth() == CV_8U && img.depth() == CV_8U &&
    ...
    // Multiply the RBG layers with the alpha channel to mask areas to be
    // ignored.
  ...
  maxVal = (double *)malloc(sizeof(double *)); // Will store best matching
                                               // score.
  ...
  rectangle(img_display, matchLoc,
            Point(matchLoc.x + slamMap.cols, matchLoc.y + slamMap.rows),
            Scalar(0, 255, 255), 2, 1, 0);
